 ...
 *
 * TIM2 used as time base for delay:
 * 	 - APB1 = 8 MHz
 * 	 - Prescaler = 8000 - 1
 *   - Timer clock = 1 kHz
 *
...
void I2C1_Init(void)
{
    ...
    GPIOB_CRL |=  (0xEE << 24);   // PB6, PB7 AF OD 2MHz
...
void DS3231_SetTime(uint8_t h, uint8_t m, uint8_t s)
{
    ...
    I2C_WriteByte(DEC_to_BCD(s) & 0x7F);  // seconds, CH=0
    I2C_WriteByte(DEC_to_BCD(m) & 0x7F);  // minutes

    // hours: 24h => bit6=0, dữ liệu BCD nằm ở bit[5:0]
    hour_bcd = DEC_to_BCD(h) & 0x3F;
    hour_bcd &= ~(1 << 6);
